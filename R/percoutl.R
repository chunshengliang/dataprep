...
#' @export
#' @noRd
percoutl <- function(data, cols = NULL, group = NULL, top = .995, bottom = .0025,
  ...
  if (is.vector(data) && !is.list(data)) {
    data <- mark_outliers_cpp(data, top, 0, 0, bottom, 0, 0, FALSE)
  ...
  if (is.null(group)) {
    ...
    a <- obsedele(a, cols = idx, by = by, half = half, date_col = date_col)
  } else {
    ...
    a <- obsedele(a, cols = idx, group = group_col, by = by, half = half, date_col = date_col)
  }
  a <- obsedele(a, cols = idx, group = group, by = by, half = half, date_col = date_col)
  if (verbose) {
    cat(sum(is.na(a[idx])) - sum(is.na(data[data[[date_name]] %in% a[[date_name]], idx])),
        "values are regarded as outliers and deleted excluding those in deleted observations\n")
    cat(nrow(data) - nrow(a), "observations are deleted in total by percoutl\n")
    cat("Time used by percoutl:", format(Sys.time() - t0, digits = 3), "\n")
  }
  a
}